...
import top.zlm.service.UserService;
 ...
 * 例：appid(com.example.demo) 则加载类 com.example.top.zlm.cq<br>
...
public class Application extends JcqAppAbstract implements ICQVer, IMsg, IRequest {
    ...
    public int startup() {
        ...
        // 返回如：D:\CoolQ\app\com.sobte.cqp.jcq\app\top.zlm.cq\
        // 应用的所有数据、配置【必须】存放于此目录，避免给用户带来困扰。
        ApplicationContext context = new AnnotationConfigApplicationContext(AppConfig.class);
        this.userService = context.getBean(UserService.class);
        this.buyService = context.getBean(BuyService.class);
        this.task = context.getBean(Task.class);
        return 0;
    }
    ...
    public int privateMsg(int subType, int msgId, long fromQQ, String msg, int font) {
        ...
        if(StringUtils.startsWith(msg,"禁言")){
            //设置群禁言
            //CQ.setGroupWholeBan(Long.parseLong(StringUtils.substring(msg,2)),true);
        }else if(StringUtils.startsWith(msg,"域名")){
            task.setHost(StringUtils.substring(msg,2));
        }
        return MSG_IGNORE;
    }
     ...
     */
    @Override
    public int groupMsg(int subType, int msgId, long fromGroup, long fromQQ,
                        String fromAnonymous, String msg,int font) {
        msg = StringUtils.trim(msg);
        if(StringUtils.equals(msg,"余") ||
                StringUtils.startsWith(msg,"上") ||
                StringUtils.startsWith(msg,"下")){
            userService.main(fromGroup,fromQQ,msg);
        }else if(StringUtils.equals(msg,"查") ||
                StringUtils.equals(msg,"清") ||
                StringUtils.startsWith(msg,"大") ||
                StringUtils.startsWith(msg,"小") ||
                StringUtils.startsWith(msg,"单") ||
                StringUtils.startsWith(msg,"双")){
            buyService.main(fromGroup,fromQQ,msg);
        }
        return MSG_IGNORE;
    }
    ...
    public int groupMemberIncrease(int subtype, int sendTime, long fromGroup, long fromQQ, long beingOperateQQ) {
        // 这里处理消息
        //入群保存新用户
        userService.save(beingOperateQQ);
        return MSG_IGNORE;
    }
